...
<head>
  ...
  <title>Confirmez votre email - GrudgeVault</title>
  <style>
...
<body>
  <div class="wrapper">
    <div class="card">
      ...
      <h1>Confirmez votre email, ${escapeHtml(params.userName)}.</h1>
      <p>Votre coffre est presque pret. Verifiez votre adresse email pour activer votre compte.</p>
      <p>Le bouton ci-dessous confirme votre adresse et active votre accés.</p>

      <a href="${params.verificationUrl}" class="cta-button">Confirmer mon email</a>

      <div class="link-box">
        <a href="${params.verificationUrl}">${escapeHtml(params.verificationUrl)}</a>
      </div>

      <p style="margin-top:20px">Si vous n'etes pas à l'origine de cette demande, ignorez simplement cet email.</p>
    </div>

    <div class="footer">
      <p>Apres verification, connexion ici: <a href="${params.loginUrl}">${params.loginUrl}</a></p>
    </div>
  </div>
</body>
</html>`
